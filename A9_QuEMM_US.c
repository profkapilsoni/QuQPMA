...
int main (int narg, char *varg[])
{
	...
	int M = 2;
	const int P_m[] = { 3, 2 };
	int P_size = 3; //Length of the longest pattern in the set  
	char P[][4] =  { {'T', 'A', 'T', '\0' }, {'C', 'C', '\0', '\0' } };
	...
	printf("Started General Classical Preprocessing..\n");
	...
	printf("Completed General Classical Preprocessing!\n");

	//ENDS - Preliminaries for quantum circuit

	//STARTS

	//Time calculcations code
	clock_t start, end;
	double time_taken = 0;
	//
    ...
    for(int np = 0; np < M; np++)
    {
	    ...
	    printf("Started Pattern Specific Classical Preprocessing..\n");
	    bin_idx = 0;
    	_bin_str = (int **)malloc(sizeof(int *) * (int)pow(2, 2 * P_m[np]));
		for(int i = 0; i < (int)pow(2, 2 * P_m[np]); i++)
			_bin_str[i] = (int *)malloc(sizeof(int) * 2 * P_m[np]);
		int *temp_a = (int *)malloc(sizeof(int) * 2 * P_m[np]); //Set accordingly = (2 * P_m[np])
		gen_all_bin_str(2 * P_m[np], temp_a, 0);
		printf("Completed Pattern Specific Classical Preprocessing!\n");
	    ...
	    printf("\nQuantum Parameters for \"%s\" to search the pattern \"%s\":\n", varg[0], P[np]);
	    ...
	    //Store T_i to T_{i + P_m[np] - 1}
	    int set_qb = n + 2 * P_m[np] - 1;
	    for(int ip = 0; ip < P_m[np]; ip++)
	    {
    	...
    	//Evaluate "T[i, i + P_m[np] - 1] == P"
		int *anc_ctrls = (int *)malloc(sizeof(int) * 4 * P_m[np]);
		set_qb = (n + 4 * P_m[np] - 1);
		int idx = 0;
		for(int i = 0; i < 2 * P_m[np]; i++)
		{
			anc_ctrls[idx++] = set_qb - i;
			anc_ctrls[idx++] = set_qb - i - (2 * P_m[np]);
		}
		for(int i = 0; i < bin_idx; i++)
		{
			int *eval_form = (int *)malloc(sizeof(int) * 4 * P_m[np]);
			int eval_idx = 0;
			for(int j = 0; j < 2 * P_m[np]; j++)
			{
				eval_form[eval_idx++] = _bin_str[i][j];
				eval_form[eval_idx++] = _bin_str[i][j];
			}
			multiStateControlledUnitary(qubits, anc_ctrls, eval_form, 4 * P_m[np], n + 4 * P_m[np], ux);
			free(eval_form);
		}
		...
		if(ch == 'y')
		{
		    ...
		    char Alpha[] = { 'A', 'T', 'C', 'G' };
		    for(int i = 0; i < (int)pow(2, n + 4 * P_m[np] + 1); i++)
		    {
		    	prob = getProbAmp(qubits, i);
		    	if(prob != 0.0)
		    	{
		    		printf("\ni = %d, T[i, i + P_m[np] - 1] = ", (i & mask));
		    		for(int j = P_m[np] - 1; j >= 0; j--)
		    			printf("%c", Alpha[(i >> (j * 2 + n)) & 0x3]);
		    		printf(", P = ");
		    		for(int j = 2 * P_m[np] - 1; j >= P_m[np]; j--)
		    			printf("%c", Alpha[(i >> (j * 2 + n)) & 0x3]);
		    		printf(", (T == P) = %d has prob %f", (i >> (n + 4 * P_m[np])), prob);
		    	}
		   	}
		}
		*/
		...
		{
		  	if(is_at(T, T_size, i, P[np], P_m[np]))
		   	{
		   		e.real[i][i] = -1;
		   		count++;
		   	}
	    ...
	    if(count != 0)
	    {
		    ...
		    {
		    	//Marking
		    	controlledMultiQubitUnitary(qubits, n + 4 * P_m[np], targs, n, e);
	...
	//Time calculcations code
	printf("\n\n----------------------------------------");
	printf("\nTime taken to simulate %f seconds.\n", time_taken);
	printf("----------------------------------------\n\n");
	//
